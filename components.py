...
def display_product(result):
    ...
    # 在庫状況
    if product['stock_status'] == ct.STOCK_FEW:
        st.warning(ct.STOCK_FEW_TEXT, icon=ct.WARNING_ICON)
    
    if product['stock_status'] == ct.STOCK_OUT:
        st.error(ct.STOCK_OUT_TEXT, icon=ct.ERROR_ICON)
